...
- **원인 후보:** ① analyze 시점에 KG 부재 → 오버레이 생략(`대시보드 오버레이: 생략` 출력). ② 시드가 KG에 미조인 → `changedNodeIds`가 비어 대시보드가 오버레이를 적용하지 않음(`주의: 시드가 KG에 매칭되지 않아…` 경고). ③ `d` 키로 diff 모드를 꺼둠. ④ `/understand-diff`를 나중에 실행해 같은 파일을 덮어씀(마지막 실행 우선).
- **대응:** analyze 출력의 `대시보드 오버레이:` 줄에서 시드/영향/미조인 수 확인. `KG 미조인 N`이 뜨면 `/understand` 분석 범위가 영향 파일을 포함하는지 확인(`diff-overlay.json`의 `ktdsImpact.unresolved`에 사유). 덮어쓰였으면 analyze 재실행(타 출처 파일은 `.bak`으로 보존됨). 브라우저는 새로고침.
